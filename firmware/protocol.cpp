...
{
    // data_len 是 $TYPE,PAYLOAD 的长度（不含校验和）
    if (data_len + 4 > buf_size) return 0; // *XX\n\0

    const char *data_start = buf + 1; // 跳过 $
    uint8_t cs = calc_checksum(data_start, data_len - 1); // -1 因为 data_len 含 $
